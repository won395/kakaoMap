...
const KakaoMap = () => {
  ...
  useEffect(() => {
    ...
    if (window.kakao) {
      ...
      window.kakao.maps.event.addListener(map, "click", (mouseEvent) => {
        ...
        addMarker(latLng, "custom"); // 클릭한 위치에 마커 추가
      });
      ...
      window.kakao.maps.event.addListener(map, "click", (mouseEvent) => {
        ...
        addMarker(latLng, markerType); // 활성 마커 타입으로 마커 추가
      });
    }
  }, [markerType]); // 컴포넌트가 마운트될 때 한 번만 실행
  ...
  const createCustomMarker = (position, map, type) => {
    const imageSrc = 
      type === "custom"
        ? "https://img.icons8.com/color/48/000000/marker.png"         //샘플 마커 이미지
        : "https://img.icons8.com/ios-filled/50/000000/marker.png";   // 기본 마커 이미지

    const imageSize = new kakao.maps.Size(48, 48);
    const imageOption = { offset: new kakao.maps.Point(24, 48) }; //이미지 중심
    ...
    const marker1 = new window.kakao.maps.Marker({
      position,
      image: markerImage,
      draggable: true, // 마커 이미지 설정
    });

    marker1.setMap(map);

    // 상태에 저장
    setMarkers((prevMarkers) => [...prevMarkers, {marker1, type}]);
  ...
  const addMarker = (position, type) => {
    ...
    createCustomMarker(position, map, type);

    // 마커 생성
    const marker = new window.kakao.maps.Marker({
      position, // 클릭한 위치 좌표
      draggable: true, // 마커가 드래그 가능하도록 설정
    });
    marker.setMap(map);
  ...
  return (
    <div>
      ...
      <h1>마커 타입 변경</h1>
        <button onClick={() => setMarkerType("default")}>기본 마커</button>{" "}
        <button onClick={() => setMarkerType("custom")}>사용자 정의 마커</button>
        <br />
